...
export default function GallerySection() {
  ...
  useEffect(() => {
    supabase.from("gallery_images").select("*").eq("is_visible", true).order("sort_order").then(({ data }) => {
    ...
    <section id="gallery" ref={ref} className="py-20 lg:py-28 bg-warm">
      <div className="max-w-7xl mx-auto section-padding">
        <div className="text-center max-w-2xl mx-auto">
          <span className={`section-label ${isVisible ? "animate-fade-in-up" : ""}`}>Gallery</span>
          ...
          <p className={`section-subtitle ${isVisible ? "animate-fade-in-up" : ""}`} style={{ animationDelay: "0.15s" }}>
            A glimpse into the vibrant campus life that makes our school special.
          </p>
        </div>

        <div className="mt-14 grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {images.map((img, i) => (
            <div key={i} className={`group relative overflow-hidden rounded-2xl aspect-[4/3] ${isVisible ? "animate-scale-in" : ""}`} style={{ animationDelay: `${0.1 + i * 0.06}s` }}>
